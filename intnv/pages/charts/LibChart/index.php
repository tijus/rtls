...
<?php
	...
	$query = "select * from rtls where time between '".convertformat($_GET["from"])."' and '".convertformat($_GET["to"])."' and rfid_no=".$_GET["no"];
	//$query = "select * from rtls where rfid_no=".$_GET["no"];
	//echo $query;
	...
	if( $num_results > 0){
		...
		//set chart title
		$chart->setTitle("Violation Status of tag ".$_GET['no']);
